...
<body>
	...
	<pre>
* 변수 선언(&lt;c:set var="변수명" value="리터럴" scope="스코프 영역지정(생략 가능)" &gt;)
- 변수를 선언하고 초기값을 대입해주는 기능을 제공
- 해당 변수를 어떤 scope에 담아둘 것인지 지정 가능함(생략시 기본적으로 pageScope에 담긴다)
=> 즉 , 해당 scope영역에 setAttribute라는 메소드를 이용해서 key + value 형태로 데이터를 담아놓는 것과 동일한 형태이다
=> c:set 을 통해 선언된 변수는 EL로 접근해서 사용 가능

* 주의사항 
- 변수의 타입을 별도로 지정하지 않음
- 반드시 해당 변수에 담아두고자 하는 초깃값(value)속성을 무조건 추가시켜줘야함(선언과 동시에 초기화해줘야함)
	</pre>
	
	<c:set var="num1" value="10"/> <!-- pageScope에 담김 -->
	
	<!-- requestScope에 담김, 주석이 코어라이브러리랑 같이 있으면 문제가 될 때가 있음 -->
	...
	<!-- 
		변수명만 제시하면 공유범위가 가장 작은곳에서부터 큰곳을 순차적으로 찾아지게됨. 티가나지는 않지만 속도가 조금 느려질 수 있다.
		따라서 스코프영역.변수명을 작성하는것이 좋다
	 -->
...
=> 즉, 해당 scope에 .removeAttribute라는 메서드를 이용해서 제거하는 것이라고 생각하면됨
	...
	<pre>
* 변수(데이터) 출력 : (&lt;c:out value="출력하고자 하는 값: default="기본값(생략가능)" escapXml="true/false(기본값:true, 생략가능)")&gt;)
...
- escapeXml : 태그로써 해석해서 출력할지 여부(생략가능, 기본값 : true)
	...
	default 설정 시 result 값 : <c:out value="${result }" default="값이 없음"/> <br><br>
...
- 각 조건들을 c:choose태그의 하위 요소로 c:when, c:otherwise를 통해서 작성
	</pre>
	...
	<c:choose>
		...
		<c:otherwise>
			<b>num1은 10혹은 2과 일치하지 않습니다</b> <br>
		</c:otherwise>
	</c:choose>
...
향상된 반복문 - (&lt;c:forEach var="변수명" items="순차적으로 접근할 배열, 컬렉션" varStatus="현재 접근된 요소에 상태값을 저장하는 변수명"&gt;)
		...
		<c:choose>
			...
			<c:otherwise>
				<c:forEach var="p" items="${requestScope.list }" varStatus="status">
					<tr align="center">
						<td>${ status.count}</td><!-- 1부터시작하는 값, index : 0부터 시작 -->
				...
				<c:forEach var="i" begin="0" end="<%= list.size() -1 %>">
					<tr align="center">
						<td>${ i +1}</td><!-- 1부터시작하는 값, index : 0부터 시작 -->
						<td>${ list[i].name}</td>
						<td>${ list[i].gender }</td>
						<td>${ list[i].job }</td>
					</tr>
...
- 특정 조건에 따라 파라미터값을 다르게 보내야 할 때 자주 사용됨
	</pre>
