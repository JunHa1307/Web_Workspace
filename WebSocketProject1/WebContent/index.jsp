...
<body>
	...
	<script>
		// 웹소켓 서버에 연결하기
		// WebSocket 객체 생성하기 192.168.30.165
		<%-- // const socket = new WebSocket("ws://ip:port/<%=request.getContextPath()%>/chatting.do") --%>
		const socket = new WebSocket("ws://192.168.30.165:8082/<%= request.getContextPath()%>/chatting.do");
		// https://ip:포트번호/~
		// http -> ws:~
		// https -> wss:~
		...
		socket.onmessage = function(e){// onmessage => sendText, sendObj 감지하는 함수
			console.log("메세지 수신");
