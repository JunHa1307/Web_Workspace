...
//		String title = request.getParameter("title");
		...
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			// 1_1. 전송파일 용량제한(int maxSize = byte단위로 값을 기술해야함) 10MB제한할 예정
			/*
			 * byte -> kbyte -> Mbyte -> Gbyte -> tByte
			 * 1Kbyte -> 1024 byte 2의 10승
			 * 1Mbyte -> 1024 Kbtye 1024 * 1024 2의 20승
			 */
			int maxSize = 1024 * 1024 * 10;
			
			// 1_2. 정달될 파일을 저장할 서버의 폴더경로 알아내기(String savePath)
			/*
			 ...
			 *  - HttpServletRequest request => MulipartRequest multi 로 변환
			 *  
			 *  매개변수 생성자로 생성(cos.jar에서 제공하는 객체)
			 *  MultipartRequest multi = new MultipartRequest(request객체, 저장할 폴더 경로, 용량 제한, 인코딩 설정값, 파일명을 수정시켜주는 객체);
			 *  
			 ...
			 *  
			 *  - 절대 파일명이 안 겹치게끔 rename해볼예정
			 *  - 즉, DefaultFilenamePolicy객체는 사용하지않음
			 *  내 입맛대로 나만의 파일명 생성규칙을 만들어서 파일이름을 바꿔누는 객체를 만들어보기
			 */
			MultipartRequest multi = new MultipartRequest(request,savePath,maxSize,"UTF-8",new MyFileRenamePolicy());
			
			// 3. DB에 기록할 데이터들을 뽑아서 Attachment객체에 담기, Board객체에 각각 담아주기
			// 	- 카테고리 번호, 제목, 내용, 작성자번호를 뽑아서 Board에 Insert
			// 	- 넘어온 첨부파일이 있다면, 원본명, 수정명, 폴더 경로를 뽑아서 Attachment테이블에 Insert
